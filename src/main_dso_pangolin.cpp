...
void settingsDefault(int preset) {
	...
	if (preset == 2 || preset == 3)
	{
		...
		setting_desiredPointDensity = 200;
		setting_minFrames = 4;
		setting_maxFrames = 6;
		...
		benchmarkSetting_width = IMAGE_WIDTH;
		benchmarkSetting_height = IMAGE_HEIGHT;
...
int main(int argc, char** argv) {
	...
	dso::setting_maxShiftWeightR = 0.0f * (IMAGE_WIDTH + IMAGE_HEIGHT);
	dso::setting_maxShiftWeightRT = 0.02f * (IMAGE_WIDTH + IMAGE_HEIGHT);
	dso::setting_desiredImmatureDensity = 450;
	dso::setting_desiredPointDensity = 450;
	dso::setting_minFrames = 2;
	dso::setting_maxFrames = 3;
	dso::setting_maxOptIterations = 2;
	dso::setting_minOptIterations = 1;
	//dso::setting_realTimeMaxKF = true;
	//dso::setting_keyframesPerSecond = 3;
